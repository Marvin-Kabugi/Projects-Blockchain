...
class Wallet:
    ...
    def save_keys(self):
        if self.public_key != None and self.private_key != None:
            try:
                with open('keys-{}.txt'.format(self.node_id), 'w') as f:
                    f.write(self.public_key)
                    f.write('\n')
                    f.write(self.private_key)
                return True
            except (IOError, IndexError):
                print('Failed saving keys')
                return False

    def load_keys(self):
        try:
            with open('keys-{}.txt'.format(self.node_id), 'r') as f:
                keys = f.readlines()
                public_key = keys[0][:-1]
                private_key = keys[1]
                self.public_key = public_key
    ...
    def sign_transaction(self, sender, recepient, amount):
        ...
        h = SHA256.new((str(sender) + str(recepient) + str(amount)).encode('utf8'))
